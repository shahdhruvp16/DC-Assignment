...
# Create Server
server = ThreadedXMLRPCServer(("localhost", 8002))
print("✓ Server 3 running on port 8002...\n")
...
def health():
    ...
    return "Server 3 (Restaurant 2) is Healthy ✓"
...
def receive_order(order_id, customer_name, items, from_server, sender_vc=None):
    ...
    # Log RECEIVE without incrementing the clock again.
    receive_vc = log_event(
        "RECEIVE",
        f"Order #{order_id} from {from_server}: "
        f"{customer_name} - Items: {items}",
    ...
    return {
        ...
        "message": "Order received at Restaurant 2"
    }
...
def prepare_order(order_id):
    ...
    log_event("INTERNAL", f"Preparing order #{order_id} in kitchen")
...
def send_delivery_update(delivery_partner, order_id):
    """Send prepared order directly from Restaurant 2 to Server-4."""
...
def receive_delivery_confirmation(
    ...
    # Update vector clock using sender's vector clock.
    if sender_vc is not None:
        vc.update(sender_vc)
    else:
        vc.increment()
...
print("  ✓ receive_delivery_confirmation(order_id, status, location)")
